...
// Age-appropriate memory sequences based on cognitive psychology research
...
const getAgeAppropriateSequences = (age: number) => {
  if (age < 7) { // Young children
    return [
      ...
      { digits: [2, 5, 1, 7, 3, 9], difficulty: 4 },
    ];
  ...
  } else if (age < 18) { // Adolescents
    return [
      { digits: [3, 1, 7, 9, 2], difficulty: 1 },
      { digits: [6, 1, 5, 8, 2, 4], difficulty: 2 },
      { digits: [8, 3, 5, 7, 9, 1, 4], difficulty: 3 },
      { digits: [2, 5, 1, 7, 3, 9, 4, 8], difficulty: 4 },
    ];
  } else if (age < 60) { // Adults
    return [
      { digits: [3, 1, 7, 9, 2, 5], difficulty: 1 },
      { digits: [6, 1, 5, 8, 2, 4, 9], difficulty: 2 },
      { digits: [8, 3, 5, 7, 9, 1, 4, 2], difficulty: 3 },
      { digits: [2, 5, 1, 7, 3, 9, 4, 8, 6], difficulty: 4 },
    ];
  } else { // Seniors
    return [
      { digits: [3, 1, 7, 9, 2], difficulty: 1 },
...
const ImprovedWorkingMemoryTest: React.FC<{ 
  onComplete: () => void; 
  onCancel: () => void; 
  userAge?: number;
  onNext?: () => void; // New prop for guided navigation
}> = ({ 
  onComplete, 
  onCancel,
  ...
  const [memorySequences, setMemorySequences] = useState(() => getAgeAppropriateSequences(userAge));
  ...
  // Update sequences if user age changes
  useEffect(() => {
    setMemorySequences(getAgeAppropriateSequences(userAge));
  }, [userAge]);
  ...
  const completeTest = () => {
    ...
    const difficultyScore = (maxCorrectDifficulty / memorySequences[memorySequences.length - 1].difficulty) * 100;
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="mt-6">
            <Button 
              ...
              disabled={userInput.length !== memorySequences[currentSequenceIndex]?.digits.length}
            >
              Submit Answer
            </Button>
          </div>
...
export default ImprovedWorkingMemoryTest;
